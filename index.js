(function() {
  ...
  var canvas, gl, program, scaleXUniformLocation, scaleX, melebar, theta, thetaSpeed, mmLoc, mm, vmLoc, vm, pmLoc, pm, camera, xHurufLocation, triangleX, yHurufLocation, triangleY, zHurufLocation, triangleZ, arahX, arahY, arahZ, dcLoc, dc, ddLoc, dd, acLoc, ac, nmLoc, vNormal, vTexCoord, vColor, flag, flagUniformLocation, fFlagUniformLocation, theta, phi;
  var verticescube = [];
  var cubePoints = [
    [ -0.8, -0.8,  0.8 ],
    [ -0.8,  0.8,  0.8 ],
  ...
  function initTexture() {
    ...
    //load image
    var image = new Image();
    image.src = "images/tes2.jpg";
    image.addEventListener('load', function() {
      //make copy to texture
      gl.bindTexture(gl.TEXTURE_2D, texture);
  ...
  function render(){
    ...
    glMatrix.mat4.lookAt(vm,
      [camera.x, camera.y, camera.z], //posisi kamera (posisi)
    ...
    if(nHuruf < 0){
      console.log('Failed to set the positions of the verticeTriangle');
      return;
    }
  ...
  function draw(){
    ...
    ac = glMatrix.vec3.fromValues(0.17, 0.41, 0.56);//nrp 05111740000080
